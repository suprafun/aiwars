	...
	for y in xrange(level.height()):
		for x in xrange(level.width()):
			...
			if ruleset.rules.has_key(terrainTypeName):
				...
				sprites = []
				(minimum, maximum, spriteRules) = ruleset.rules[terrainTypeName]
				for i in xrange(random.randint(minimum, maximum)):
					spriteRule = random.choice(spriteRules)
					spriteX = x * tileSize.x + random.randint(0, tileSize.x - spriteRule.boundingBox[2]) - spriteRule.boundingBox[0]
					spriteY = y * tileSize.y + random.randint(0, tileSize.y - spriteRule.boundingBox[3]) - spriteRule.boundingBox[1]
